;;; aiern-types.el --- Type system -*- lexical-binding: t -*-
                ...
                ((progn
                 ...
                 (cond
                  ((progn
                   ...
                   (aiern-expand beg end 'line))
                  (t
                   (unless aiern-cross-lines
                     (setq end (max beg (1- end))))
                   (aiern-expand beg end 'inclusive))))
                (t
                 (aiern-range beg end))))
...
(aiern-define-type inclusive
  "Include the character under point.
If the end position is at the beginning of a line or the end of a
line and `aiern-want-visual-char-semi-exclusive', then:

* If in visual state return `exclusive' (expanded)."
  :expand (lambda (beg end)
            (if (and aiern-want-visual-char-semi-exclusive
                     (aiern-visual-state-p)
                     (< beg end)
                     (save-excursion
                       (goto-char end)
                       (or (bolp) (eolp))))
                (aiern-range beg end 'exclusive)
              (aiern-range beg (1+ end))))
  :contract (lambda (beg end)
              (aiern-range beg (max beg (1- end))))
  :normalize (lambda (beg end)
               (goto-char end)
               (when (eq (char-after) ?\n)
                 (setq end (max beg (1- end))))
...
(aiern-define-type screen-line
  "Include whole lines, being aware of `visual-line-mode'
when `aiern-respect-visual-line-mode' is non-nil."
  :one-to-one nil
  :expand (lambda (beg end)
            (if (or (not aiern-respect-visual-line-mode)
                    (not visual-line-mode))
                (aiern-line-expand beg end)
              (aiern-range
               (progn
                 (goto-char beg)
                 (save-excursion
...
and `lower-right'."
            (let ((left  (aiern-column beg))
                  (right (aiern-column end))
                  (corner (or (plist-get properties :corner)
                              'upper-left)))
              (aiern-sort left right)
              (goto-char beg)
              ...
              (setq properties (plist-put properties
                                          :corner corner))
              (apply #'aiern-range beg end properties))))

(aiern-define-type rectangle
  "Like `exclusive', but for rectangles:
the last column is excluded."
  :expand (lambda (beg end)
            ;; select at least one column
            (if (= (aiern-column beg) (aiern-column end))
                (aiern-expand beg end 'block)
              (aiern-range beg end 'block))))

;;; Standard interactive codes

(aiern-define-interactive-code "*"
  "Signal error if the buffer is read-only."
  (when buffer-read-only
    (signal 'buffer-read-only nil)))

(aiern-define-interactive-code "b" (prompt)
  "Name of existing buffer."
  (list (read-buffer prompt (current-buffer) t)))

(aiern-define-interactive-code "c"
  "Read character."
  (list (read-char)))

(aiern-define-interactive-code "p"
...
(aiern-define-interactive-code "<vc>"
...
directly."
  (list (when (and (aiern-visual-state-p) current-prefix-arg)
          (prefix-numeric-value
           current-prefix-arg))))

(aiern-define-interactive-code "<C>"
  "Character read through `aiern-read-key'."
  (list
   (if (aiern-operator-state-p)
       (aiern-without-restriction (aiern-read-key))
     (aiern-read-key))))

(aiern-define-interactive-code "<r>"
  "Untyped motion range (BEG END)."
  (aiern-operator-range))

(aiern-define-interactive-code "<R>"
  "Typed motion range (BEG END TYPE)."
  (aiern-operator-range t))

(aiern-define-interactive-code "<v>"
...
(aiern-define-interactive-code "<x>"
  "Current register."
  (list aiern-this-register))

(aiern-define-interactive-code "<y>"
  "Current yank-handler."
  (list (aiern-yank-handler)))

(aiern-define-interactive-code "<a>"
  "Ex argument."
  :ex-arg t
  (list (when (aiern-ex-p) aiern-ex-argument)))

(aiern-define-interactive-code "<N>" (prompt)
  "Prefix argument, ex-arg or minibuffer input, converted to number"
  (list (cond
         (current-prefix-arg (prefix-numeric-value current-prefix-arg))
         ((and aiern-ex-argument (aiern-ex-p)) (string-to-number aiern-ex-argument))
         ((aiern-ex-p) nil)
         (t (string-to-number (read-string prompt))))))

(aiern-define-interactive-code "<f>"
  "Ex file argument."
  :ex-arg file
  (list (when (aiern-ex-p) (aiern-ex-file-arg))))

(aiern-define-interactive-code "<b>"
  "Ex buffer argument."
  :ex-arg buffer
  (list (when (aiern-ex-p) aiern-ex-argument)))

(aiern-define-interactive-code "<sh>"
  "Ex shell command argument."
  :ex-arg shell
  (list (when (aiern-ex-p) aiern-ex-argument)))

(aiern-define-interactive-code "<fsh>"
  "Ex file or shell command argument."
  :ex-arg file-or-shell
  (list (when (aiern-ex-p) aiern-ex-argument)))

(aiern-define-interactive-code "<sym>"
  "Ex symbolic argument."
  :ex-arg sym
  (list (when (and (aiern-ex-p) aiern-ex-argument)
          (intern aiern-ex-argument))))

(aiern-define-interactive-code "<addr>"
  "Ex line number."
  (list
   (and (aiern-ex-p)
        (let ((expr (aiern-ex-parse  aiern-ex-argument)))
          (if (eq (car expr) 'aiern-goto-line)
              (save-excursion
                (goto-char aiern-ex-point)
                (eval (cadr expr)))
...
(aiern-define-interactive-code "<!>"
  "Ex bang argument."
  :ex-bang t
  (list (when (aiern-ex-p) aiern-ex-bang)))

(aiern-define-interactive-code "</>"
  "Ex delimited argument."
  (when (aiern-ex-p)
    (aiern-delimited-arguments aiern-ex-argument)))

(aiern-define-interactive-code "<g/>"
  "Ex global argument."
  (when (aiern-ex-p)
    (aiern-ex-parse-global aiern-ex-argument)))

(aiern-define-interactive-code "<s/>"
  "Ex substitution argument."
  :ex-arg substitution
  (when (aiern-ex-p)
    (aiern-ex-get-substitute-info aiern-ex-argument t)))

(aiern-define-interactive-code "<xc/>"
...
REGISTER"
  (when (aiern-ex-p)
    (aiern-ex-get-optional-register-and-count aiern-ex-argument)))

(defun aiern-ex-get-optional-register-and-count (string)
