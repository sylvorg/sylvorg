...
parser.add_argument("-r", "--rsync-both", action="store_true")
parser.add_argument("--rsync-system", action="store_true")
parser.add_argument("--rsync-mount", action="store_true")
...
resources = f'{os.environ["HOME"]}/shadowrylander/etc/nixos/config'
...
if args.subcommand == "mount":
    ...
    if not os.path.isdir("/shadowrylander/tmp"):
        run(f"mkdir -p /shadowrylander/tmp", shell = True)
    if not os.path.isdir("/shadowrylander/nix"):
        run(f"mkdir -p /shadowrylander/nix", shell = True)
    run(f"mount --bind /shadowrylander/tmp /tmp", shell = True)
    run(f"rsync -avvczz --delete /nix/ /shadowrylander/nix/", shell = True)
    run(f"mount --bind /shadowrylander/nix /nix", shell = True)

if args.generate:
    run("nixos-generate-config --root /mnt", shell = True)

if args.rsync-mount:
    run("rsync -avvczz ~/shadowrylander/ /mnt/", shell = True)

if args.rsync-system:
    run("rsync -avvczz ~/shadowrylander/ /", shell = True)
...
if args.bootstrap:
    run("nix-channel --add https://github.com/nixos/nixpkgs/archive/master/nixpkgs.tar.gz master", shell = True)
    run("nix-channel --update", shell = True)
    run("nix-env -iA master.emacs master.python310", shell = True)
    run("chmod +x ~/shadowrylander/wheee.py", shell = True)
    run("git -C ~/shadowrylander/settings checkout main", shell = True)
    run("~/shadowrylander/settings/org-tangle.sh oreo.aiern.org README.org", shell = True)
    run("git clone https://github.com/shadowrylander/shadowrylander ~/shadowrylander/shadowrylander", shell = True)
