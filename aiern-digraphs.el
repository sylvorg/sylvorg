;;; aiern-digraphs.el --- Digraphs -*- lexical-binding: t -*-
...
(defgroup aiern-digraphs nil
  "Digraph support based on RFC 1345."
  :group 'aiern
  :prefix "aiern-digraph-")

(defcustom aiern-digraphs-table-user nil
...
See also `aiern-digraphs-table'."
  :type '(alist :key-type (list character character)
                :value-type character)
  :require 'aiern-digraphs
  :group 'aiern-digraphs)

(defconst aiern-digraphs-table
  '(((?N ?U) . ?\x00)
    ((?S ?H) . ?\x01)
    ((?S ?X) . ?\x02)
    ((?E ?X) . ?\x03)
...
See also `aiern-digraphs-table-user'.")

(defun aiern-digraph (digraph)
...
Searches in `aiern-digraphs-table-user' and `aiern-digraphs-table'."
  (if (listp digraph)
      (let* ((char1 (car digraph))
             (char2 (cadr digraph)))
        (or (cdr (assoc (list char1 char2) aiern-digraphs-table-user))
            (cdr (assoc (list char1 char2) aiern-digraphs-table))
            (unless (eq char1 char2)
              (or (cdr (assoc (list char2 char1) aiern-digraphs-table-user))
                  (cdr (assoc (list char2 char1) aiern-digraphs-table))))))
    (or (car (rassoc digraph aiern-digraphs-table-user))
        (car (rassoc digraph aiern-digraphs-table)))))

(provide 'aiern-digraphs)

;;; aiern-digraphs.el ends here
