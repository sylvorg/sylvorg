;;; aiern-core.el --- Core functionality -*- lexical-binding: t -*-
...
;; aiern is defined as a globalized minor mode, enabled with the toggle
;; function `aiern-mode'.  This in turn enables `aiern-local-mode' in
...
;;
;; Within this hierarchy, aiern arranges the keymaps for the current
...
;;
;; These keymaps are listed in `aiern-mode-map-alist', which is listed
;; in `emulation-mode-map-alist'.
;;
;; Most of the key bindings for a state are stored in its global
;; keymap, which has a name such as `aiern-normal-state-map'. (See the
;; file aiern-maps.el, which contains all the default key bindings.) A
;; state also has a local keymap (`aiern-normal-state-local-map'),
...
;; own by passing the mode's keymap to the function `aiern-define-key'
;; or `aiern-define-minor-mode-key'. The former uses a specific map to
...
;; to emulation status by passing it to `aiern-make-intercept-map' or
;; `aiern-make-overriding-map'.  An "intercept" keymap has priority over
;; all other aiern keymaps.  (aiern uses this facility when debugging and
...
;; shadowed by aiern's default bindings.)
;;
;; States are defined with the macro `aiern-define-state', which
;; creates a command for switching to the state.  This command,
;; for example `aiern-normal-state' for Normal state, performs
;; the following tasks:
;;
;;     * Setting `aiern-state' to the new state.
;;     * Refreshing the keymaps in `aiern-mode-map-alist'.
;;     * Updating the mode line.
;;       - Normal state depends on `aiern-normal-state-tag'.
;;     * Adjusting the cursor's appearance.
;;       - Normal state depends on `aiern-normal-state-cursor'.
;;     * Displaying a message in the echo area.
;;       - Normal state depends on `aiern-normal-state-message'.
;;     * Running hooks.
;;       - Normal state runs `aiern-normal-state-entry-hook' when
;;         entering, and `aiern-normal-state-exit-hook' when exiting.
;;
...
;; to the `aiern-state-property' function.  aiern defines the states
...
(defvar aiern-mode-buffers)

(define-minor-mode aiern-local-mode
  "Minor mode for setting up aiern in a single buffer."
  :init-value nil
  (cond
   ((aiern-disabled-buffer-p)
    ;; Don't leave the mode variable on in buffers where aiern disabled, because
    ;; functions that check this variable will get an incorrect result (e.g.,
    ;; aiern-refresh-cursor).
    (setq aiern-local-mode nil))
   (aiern-local-mode
    (setq emulation-mode-map-alists
          (aiern-concat-lists '(aiern-mode-map-alist)
                             emulation-mode-map-alists))
    (aiern-initialize-local-keymaps)
    ;; restore the proper value of `major-mode' in Fundamental buffers
    (when (eq major-mode 'turn-on-aiern-mode)
      (setq major-mode 'fundamental-mode))
    (when (minibufferp)
      (setq-local aiern-default-state 'insert)
      (setq-local aiern-echo-state nil))
    ;; The initial state is usually setup by `aiern-initialize' when
    ;; the major-mode in a buffer changes. This preliminary
    ;; initialization is only for the case when `aiern-local-mode' is
    ;; called directly for the first time in a buffer.
    (unless aiern-state (aiern-initialize-state))
    (add-hook 'input-method-activate-hook 'aiern-activate-input-method t t)
    (add-hook 'input-method-deactivate-hook 'aiern-deactivate-input-method t t)
    (add-hook 'activate-mark-hook 'aiern-visual-activate-hook nil t)
    (add-hook 'pre-command-hook 'aiern-repeat-pre-hook)
    (add-hook 'post-command-hook 'aiern-repeat-post-hook))
   (t
    (aiern-refresh-mode-line)
    (remove-hook 'activate-mark-hook 'aiern-visual-activate-hook t)
    (remove-hook 'input-method-activate-hook 'aiern-activate-input-method t)
    (remove-hook 'input-method-deactivate-hook 'aiern-deactivate-input-method t)
    (aiern-change-state nil))))
...
;; variables.
(put 'aiern-local-mode 'permanent-local t)

(defun turn-on-aiern-mode (&optional arg)
  "Turn on aiern in the current buffer."
  (interactive)
  (aiern-local-mode (or arg 1)))

(defun turn-off-aiern-mode (&optional arg)
  "Turn off aiern in the current buffer."
  (interactive)
  (aiern-local-mode (or arg -1)))

;; The function `aiern-initialize' should only be used to initialize
;; `aiern-local-mode' from the globalized minor-mode `aiern-mode'. It is
;; called whenever aiern is enabled in a buffer for the first time or
;; when aiern is active and the major-mode of the buffer changes. In
;; addition to enabling `aiern-local-mode' it also sets the initial
;; aiern-state according to the major-mode.
(defun aiern-initialize ()
  "Enable aiern in the current buffer, if appropriate.
To enable aiern globally, do (aiern-mode 1)."
  (unless (and (minibufferp) (not aiern-want-minibuffer))
    (aiern-local-mode 1)
    (aiern-initialize-state)))

;;;###autoload (autoload 'aiern-mode "aiern" nil t)
(define-globalized-minor-mode aiern-mode
  aiern-local-mode aiern-initialize)

;; No hooks are run in Fundamental buffers, so other measures are
;; necessary to initialize aiern in these buffers. When aiern is
;; enabled globally, the default value of `major-mode' is set to
;; `turn-on-aiern-mode', so that aiern is enabled in Fundamental
;; buffers as well. Then, the buffer-local value of `major-mode' is
;; changed back to `fundamental-mode'. (Since the `aiern-mode' function
;; is created by a macro, we use `defadvice' to augment it.)
(defadvice aiern-mode (after start-aiern activate)
  "Enable aiern in Fundamental mode."
  (if aiern-mode
      (progn
        (when (eq (default-value 'major-mode) 'fundamental-mode)
          ;; changed back by `aiern-local-mode'
          (setq-default major-mode 'turn-on-aiern-mode))
        (ad-enable-regexp "^aiern")
        (ad-activate-regexp "^aiern")
        (with-no-warnings (aiern-esc-mode 1)))
    (when (eq (default-value 'major-mode) 'turn-on-aiern-mode)
      (setq-default major-mode 'fundamental-mode))
    (ad-disable-regexp "^aiern")
    (ad-update-regexp "^aiern")
    (with-no-warnings (aiern-esc-mode -1))))

(defun aiern-change-state (state &optional message)
  "Change the state to STATE.
If STATE is nil, disable all states."
  (let ((func (aiern-state-property (or state aiern-state) :toggle)))
    (when (and (functionp func)
               (or message (not (eq state aiern-state))))
      (funcall func (if state (and message 1) -1)))))

(defmacro aiern-save-state (&rest body)
  "Save the current state; execute BODY; restore the state."
  (declare (indent defun)
           (debug t))
  `(let* ((aiern-state aiern-state)
...
(defmacro aiern-with-state (state &rest body)
...
(defun aiern-initializing-p (&optional buffer)
  "Whether aiern is in the process of being initialized."
  (memq (or buffer (current-buffer)) aiern-mode-buffers))

(defun aiern-initialize-state (&optional state buffer)
  "Set up the initial state for BUFFER.
BUFFER defaults to the current buffer.
Uses STATE if specified, or calls `aiern-initial-state-for-buffer'.
See also `aiern-set-initial-state'."
  (with-current-buffer (or buffer (current-buffer))
    (if state (aiern-change-state state)
      (aiern-change-to-initial-state buffer))))
(put 'aiern-initialize-state 'permanent-local-hook t)

(defun aiern-initial-state-for-buffer-name (&optional name default)
  "Return the initial aiern state to use for a buffer with name NAME.
Matches the name against the regular expressions in
`aiern-buffer-regexps'. If none matches, returns DEFAULT."
  (let ((name (if (stringp name) name (buffer-name name)))
        regexp state)
    (when (stringp name)
      (catch 'done
        (dolist (entry aiern-buffer-regexps default)
          (setq regexp (car entry)
                state (cdr entry))
          (when (string-match regexp name)
            (throw 'done state)))))))

(defun aiern-disabled-buffer-p (&optional buffer)
  "Whether aiern should be disabled in BUFFER."
  (null (aiern-initial-state-for-buffer-name buffer 'undefined)))

(defun aiern-initial-state-for-buffer (&optional buffer default)
  "Return the initial aiern state to use for BUFFER.
BUFFER defaults to the current buffer. Returns DEFAULT
if no initial state is associated with BUFFER.
See also `aiern-initial-state'."
  (with-current-buffer (or buffer (current-buffer))
    (or (aiern-initial-state-for-buffer-name (buffer-name))
        (catch 'done
          (dolist (mode minor-mode-map-alist)
            (setq mode (car-safe mode))
            (when (and (boundp mode) (symbol-value mode))
              (when (setq mode (aiern-initial-state mode))
                (throw 'done mode)))))
        (aiern-initial-state major-mode nil t)
        default)))

(defun aiern-initial-state (mode &optional default follow-parent checked-modes)
  "Return the aiern state to use for MODE or its alias.
Returns DEFAULT if no initial state is associated with MODE.
The initial state for a mode can be set with
`aiern-set-initial-state'.
  ...
  (cond
   ...
   ((and mode (symbolp mode))
    (let ((mode-alias (let ((func (symbol-function mode)))
      ...
      (or
       (catch 'done
         (dolist (entry (aiern-state-property t :modes) default)
           ...
           (when (or (memq mode modes)
             ...
             (throw 'done state))))
       (when follow-parent
         (aiern-initial-state (get mode 'derived-mode-parent)
                             nil t (cons mode checked-modes)))
       (when follow-parent
         (aiern-initial-state (get mode-alias 'derived-mode-parent)
                             nil t (cons mode-alias checked-modes))))))))

(defun aiern-set-initial-state (mode state)
  "Set the initial state for major mode MODE to STATE.
This is the state the buffer comes up in."
  (dolist (modes (aiern-state-property t :modes))
    (setq modes (cdr-safe modes))
    (set modes (delq mode (symbol-value modes))))
  (when state
    (add-to-list (aiern-state-property state :modes) mode)))

(aiern-define-command aiern-change-to-initial-state
  (&optional buffer message)
  "Change the state of BUFFER to its initial state.
This is the state the buffer came up in. If aiern is not activated
then this function does nothing."
  :keep-visual t
  :suppress-operator t
  (with-current-buffer (or buffer (current-buffer))
    (when aiern-local-mode
      (aiern-change-state (aiern-initial-state-for-buffer
                          buffer (or aiern-default-state 'normal))
                         message))))

(aiern-define-command aiern-change-to-previous-state
  (&optional buffer message)
  ...
  :suppress-operator t
  (with-current-buffer (or buffer (current-buffer))
    (let ((prev-state aiern-previous-state)
          (prev-prev-state (cdr-safe (assoc aiern-previous-state
                                            aiern-previous-state-alist))))
      (aiern-change-state nil)
      (when prev-prev-state
        (setq aiern-previous-state prev-prev-state))
      (aiern-change-state (or prev-state aiern-default-state 'normal)
                         message))))

;; When a buffer is created in a low-level way, it is invisible to
;; aiern (as well as other globalized minor modes) because no hooks are
...
;; otherwise, though, so advise this function to initialize aiern.
(defadvice set-window-buffer (before aiern)
  "Initialize aiern in the displayed buffer."
  (when aiern-mode
    (when (get-buffer (ad-get-arg 1))
      (with-current-buffer (ad-get-arg 1)
        (unless aiern-local-mode
          (aiern-local-mode 1))))))

;; Refresh cursor color.
;; Cursor color can only be set for each frame but not for each buffer.
(add-hook 'window-configuration-change-hook 'aiern-refresh-cursor)
(defadvice select-window (after aiern activate)
  (aiern-refresh-cursor))

(defun aiern-generate-mode-line-tag (&optional state)
  "Generate the aiern mode-line tag for STATE."
  (let ((tag (aiern-state-property state :tag t)))
    (when (functionp tag)
      (setq tag (funcall tag)))
    ;; prepare mode-line: add tooltip
    (if (stringp tag)
        (propertize tag
                    'help-echo (aiern-state-property state :name)
                    'mouse-face 'mode-line-highlight)
      tag)))

(defun aiern-refresh-mode-line (&optional state)
  "Refresh mode line tag."
  (when (listp mode-line-format)
    (setq aiern-mode-line-tag (aiern-generate-mode-line-tag state))
    ;; refresh mode line data structure
    ;; first remove aiern from mode-line
    (setq mode-line-format (delq 'aiern-mode-line-tag mode-line-format))
    (let ((mlpos mode-line-format)
          pred which where)
      ;; determine before/after which symbol the tag should be placed
      (cond
       ((eq aiern-mode-line-format 'before)
        (setq where 'after which 'mode-line-position))
       ((eq aiern-mode-line-format 'after)
        (setq where 'after which 'mode-line-modes))
       ((consp aiern-mode-line-format)
        (setq where (car aiern-mode-line-format)
              which (cdr aiern-mode-line-format))))
      ...
      (while (and mlpos
        ...
        (setq pred mlpos
              mlpos (cdr mlpos)))
      ;; put aiern tag at the right position in the mode line
      (cond
       ((not mlpos)) ;; position not found, so do not add the tag
       ((eq where 'before)
        (if pred
            (setcdr pred (cons 'aiern-mode-line-tag mlpos))
          (setq mode-line-format
                (cons 'aiern-mode-line-tag mode-line-format))))
       ((eq where 'after)
        (setcdr mlpos (cons 'aiern-mode-line-tag (cdr mlpos)))))
      (force-mode-line-update))))

;; input methods should be disabled in non-insertion states
(defun aiern-activate-input-method ()
  "Enable input method in states with :input-method non-nil."
  (let (input-method-activate-hook
        input-method-deactivate-hook)
    (when (and aiern-local-mode aiern-state)
      (setq aiern-input-method current-input-method)
      (unless (aiern-state-property aiern-state :input-method)
        (deactivate-input-method)))))
(put 'aiern-activate-input-method 'permanent-local-hook t)

(defun aiern-deactivate-input-method ()
  "Disable input method in all states."
  (let (input-method-activate-hook
        input-method-deactivate-hook)
    (when (and aiern-local-mode aiern-state)
      (setq aiern-input-method nil))))
(put 'aiern-deactivate-input-method 'permanent-local-hook t)

(defmacro aiern-without-input-method-hooks (&rest body)
  "Execute body with aiern's activate/deactivate-input-method hooks deactivated.
  ...
  `(unwind-protect
       (progn
         ...
         (remove-hook 'input-method-deactivate-hook
                      'aiern-deactivate-input-method t)
         ,@body)
     (progn
       (add-hook 'input-method-activate-hook 'aiern-activate-input-method nil t)
       (add-hook 'input-method-deactivate-hook
                 'aiern-deactivate-input-method nil t))))

(defadvice toggle-input-method (around aiern)
  "Refresh `aiern-input-method'."
  (cond
   ((not aiern-local-mode)
    ad-do-it)
   ((aiern-state-property aiern-state :input-method)
    ad-do-it)
   (t
    (let ((current-input-method aiern-input-method))
      ad-do-it))))
...
higher precedence. See also `aiern-make-intercept-map'."
...
(defun aiern-make-intercept-map (keymap &optional state aux)
  "Give KEYMAP precedence over all aiern keymaps in STATE.
...
KEYMAP itself. See also `aiern-make-overriding-map'."
...
(defmacro aiern-define-keymap (keymap doc &rest body)
  "Define a keymap KEYMAP listed in `aiern-mode-map-alist'.
...
\(fn KEYMAP DOC [[KEY VAL]...] BODY...)"
  ...
  (let ((func t)
    ...
    `(progn
       ...
       (when ,intercept
         (aiern-make-intercept-map ,keymap))
       (when ,overriding
         (aiern-make-overriding-map ,keymap))
       ,@(if local
             `((make-variable-buffer-local ',keymap)
               (put ',keymap 'permanent-local t)
               (aiern--add-to-alist 'aiern-local-keymaps-alist
                                  ',mode ',keymap))
           `((aiern--add-to-alist 'aiern-global-keymaps-alist
                                ',mode ',keymap)
             (aiern--add-to-alist 'aiern-mode-map-alist
                                ',mode ,keymap)))
       ,(when (or body func)
...
(defun aiern-esc-mode (&optional arg)
  "Toggle interception of \\e (escape).
Enable with positive ARG and disable with negative ARG.

When enabled, `aiern-esc-mode' modifies the entry of \\e in
`input-decode-map'. If such an event arrives, it is translated to
a plain 'escape event if no further event occurs within
`aiern-esc-delay' seconds. Otherwise no translation happens and
the ESC prefix map (i.e. the map originally bound to \\e in
`input-decode-map`) is returned."
  (cond
   ((or (null arg) (eq arg 0))
    (aiern-esc-mode (if aiern-esc-mode -1 +1)))
   ((> arg 0)
    (unless aiern-esc-mode
      (setq aiern-esc-mode t)
      (add-hook 'after-make-frame-functions #'aiern-init-esc)
      (mapc #'aiern-init-esc (frame-list))))
   ((< arg 0)
    (when aiern-esc-mode
      (remove-hook 'after-make-frame-functions #'aiern-init-esc)
      (mapc #'aiern-deinit-esc (frame-list))
      (setq aiern-esc-mode nil)))))

(defun aiern-init-esc (frame)
  "Update `input-decode-map' in terminal."
  (with-selected-frame frame
    (let ((term (frame-terminal frame)))
      (when (and
             (or (eq aiern-intercept-esc 'always)
                 (and aiern-intercept-esc
                      (eq (terminal-live-p term) t))) ; only patch tty
             (not (terminal-parameter term 'aiern-esc-map)))
        (let ((aiern-esc-map (lookup-key input-decode-map [?\e])))
          (set-terminal-parameter term 'aiern-esc-map aiern-esc-map)
          (define-key input-decode-map [?\e]
            `(menu-item "" ,aiern-esc-map :filter ,#'aiern-esc)))))))

(defun aiern-deinit-esc (frame)
  "Restore `input-decode-map' in terminal."
  (with-selected-frame frame
    (let ((term (frame-terminal frame)))
      (when (terminal-live-p term)
        (let ((aiern-esc-map (terminal-parameter term 'aiern-esc-map)))
          (when aiern-esc-map
            (define-key input-decode-map [?\e] aiern-esc-map)
            (set-terminal-parameter term 'aiern-esc-map nil)))))))
...
this function waits for `aiern-esc-delay' seconds for another
event. If no other event arrives, the event is translated to
'escape, otherwise it is translated to the standard ESC prefix
map stored in `input-decode-map'. If `aiern-inhibit-esc' is
non-nil or if aiern is in emacs state, the event is always
...
mapping will always be the ESC prefix map."
  (if (and (not aiern-inhibit-esc)
           (or aiern-local-mode (aiern-ex-p)
               (active-minibuffer-window))
           (not (aiern-emacs-state-p))
           (let ((keys (this-single-command-keys)))
             (and (> (length keys) 0)
                  (= (aref keys (1- (length keys))) ?\e)))
           (sit-for aiern-esc-delay))
      (prog1 [escape]
...
(defun aiern-state-p (sym)
  "Whether SYM is the name of a state."
  (assq sym aiern-state-properties))

(defun aiern-state-keymaps (state &rest excluded)
...
infinite recursion, keeping track of processed states.)"
  (let* ((state (or state aiern-state))
         (enable (aiern-state-property state :enable))
         (map (cons
               (aiern-state-property state :mode)
               (aiern-state-property state :keymap t)))
         (local-map (cons
                     (aiern-state-property state :local)
                     (aiern-state-property state :local-keymap t)))
         (minor-mode-maps (aiern-state-minor-mode-keymaps state))
         (aux-maps (aiern-state-auxiliary-keymaps state))
         (overriding-maps
          (aiern-state-overriding-keymaps state))
         (intercept-maps
          (aiern-state-intercept-keymaps state))
      ...
      (cond
       ...
       ((eq entry state)
        (setq result `(,@result
                       ...
                       (,map)))
        (push state excluded))
       ;; the keymaps for another state: call `aiern-state-keymaps'
       ;; recursively, but keep track of processed states
       ((aiern-state-p entry)
        (setq result `(,@result
                       ,(apply #'aiern-state-keymaps entry excluded))))
       ;; a single keymap
       ((or (keymapp entry)
            (and (keymapp (symbol-value entry))
                 (setq entry (symbol-value entry)))
            (setq entry (aiern-keymap-for-mode entry)))
        (setq result `(,@result
                       ((,(aiern-mode-for-keymap entry t) .
                         ,entry)))))))
    ;; postpone the expensive filtering of duplicates to the top level
    (if remove-duplicates
        (apply #'aiern-concat-keymap-alists result)
      (apply #'append result))))

(defun aiern-normalize-keymaps (&optional state)
  "Create a buffer-local value for `aiern-mode-map-alist'.
...
\(or by STATE if specified)."
  (let ((state (or state aiern-state))
    ...
    (dolist (entry aiern-mode-map-alist)
      ...
      (if (or (memq mode excluded)
              (aiern-intercept-keymap-p map)
              (aiern-overriding-keymap-p map)
              (aiern-auxiliary-keymap-p map)
              (aiern-minor-mode-keymap-p map))
          (push mode excluded)
        (when (and (fboundp mode) (symbol-value mode))
          (funcall mode -1))
        (set mode nil)))
    (setq aiern-mode-map-alist nil)
    ;; activate keymaps of current state
    (when state
      (setq temp (aiern-state-keymaps state))
      (dolist (entry temp)
        (setq mode (car entry)
              map (cdr entry))
        ...
        ;; (e.g., `aiern-operator-shortcut-map' is
        ;; reset on toggling)
        (if (or (memq mode excluded)
                (aiern-intercept-keymap-p map)
                (aiern-overriding-keymap-p map)
                ...
                (aiern-minor-mode-keymap-p map))
            (push mode excluded)
          (setcdr entry (or (aiern-keymap-for-mode mode) map))))
      ;; update `aiern-mode-map-alist'
      (setq aiern-mode-map-alist temp))))

(defun aiern-mode-for-keymap (keymap &optional default)
  "Return the minor mode associated with KEYMAP.
Returns DEFAULT if no mode is found.
See also `aiern-keymap-for-mode'."
  (let ((map (if (keymapp keymap) keymap (symbol-value keymap)))
        (var (when (symbolp keymap) keymap)))
    ;; Check aiern variables first for speed purposes.
    ;; If all else fails, check `minor-mode-map-alist'.
    (or (when var
          (or (car (rassq var aiern-global-keymaps-alist))
              (car (rassq var aiern-local-keymaps-alist))))
        (car (rassq map (mapcar #'(lambda (e)
                                    ;; from (MODE-VAR . MAP-VAR)
                                    ;; to (MODE-VAR . MAP)
                                    (cons (car-safe e)
                                          (symbol-value (cdr-safe e))))
                                (append aiern-global-keymaps-alist
                                        aiern-local-keymaps-alist))))
        (car (rassq map minor-mode-map-alist))
        default)))

(defun aiern-keymap-for-mode (mode &optional variable)
  "Return the keymap associated with MODE.
Return the keymap variable if VARIABLE is non-nil.
See also `aiern-mode-for-keymap'."
  (let* ((var (or (cdr (assq mode aiern-global-keymaps-alist))
                  (cdr (assq mode aiern-local-keymaps-alist))))
         (map (or (symbol-value var)
                  (cdr (assq mode minor-mode-map-alist)))))
    (if variable var map)))

(defun aiern-state-auxiliary-keymaps (state)
  "Return a keymap alist of auxiliary keymaps for STATE."
  (let ((state (or state aiern-state))
        aux result)
    (dolist (map (current-active-maps) result)
      (when (setq aux (aiern-get-auxiliary-keymap map state))
        (push (cons (aiern-mode-for-keymap map t) aux) result)))
    (nreverse result)))

(defun aiern-state-minor-mode-keymaps (state)
  "Return a keymap alist of minor-mode keymaps for STATE."
  (let* ((state (or state aiern-state))
         (state-entry (assq state aiern-minor-mode-keymaps-alist)))
    (when state-entry
      (cdr state-entry))))

(defun aiern-state-overriding-keymaps (&optional state)
  "Return a keymap alist of overriding keymaps for STATE."
  (let* ((state (or state aiern-state))
         result)
    (dolist (map (current-active-maps))
      (when (setq map (aiern-overriding-keymap-p map state))
        (push (cons (aiern-mode-for-keymap map t) map) result)))
    (nreverse result)))

(defun aiern-state-intercept-keymaps (&optional state)
  "Return a keymap alist of intercept keymaps for STATE."
  (let* ((state (or state aiern-state))
         result)
    (dolist (map (current-active-maps))
      (when (setq map (or (aiern-intercept-keymap-p map state)
                          (aiern-intercept-keymap-p
                           (aiern-get-auxiliary-keymap map state) state)))
        (push (cons (aiern-mode-for-keymap map t) map) result)))
    (setq result (nreverse result))
    result))

(defun aiern-set-auxiliary-keymap (map state &optional aux)
  "Set the auxiliary keymap for MAP in STATE to AUX.
If AUX is nil, create a new auxiliary keymap."
  (unless (keymapp aux)
    (setq aux (make-sparse-keymap)))
  (unless (aiern-auxiliary-keymap-p aux)
    (aiern-set-keymap-prompt
     aux (format "Auxiliary keymap for %s"
                 (or (aiern-state-property state :name)
                     (format "%s state" state)))))
  (define-key map
    (vconcat (list (intern (format "%s-state" state)))) aux)
  aux)
(put 'aiern-set-auxiliary-keymap 'lisp-indent-function 'defun)

(defun aiern-get-auxiliary-keymap (map state &optional create ignore-parent)
    ...
    (let* ((key (vconcat (list (intern (format "%s-state" state)))))
      ...
      (cond
       ((and ignore-parent
             ...
             create)
        (aiern-set-auxiliary-keymap map state))
       ((aiern-auxiliary-keymap-p aux)
        aux)
       (create
        (aiern-set-auxiliary-keymap map state))))))

(defun aiern-get-minor-mode-keymap (state mode)
  "Get the auxiliary keymap for MODE in STATE, creating one if it
does not already exist."
  (let ((state-entry (assq state aiern-minor-mode-keymaps-alist)))
    (if (and state-entry
...
(defun aiern-overriding-keymap-p (map &optional state)
  ...
  (let ((entry (and (keymapp map)
    ...
    (cond
     ((null entry)
      nil)
     ((keymapp entry)
      (aiern-overriding-keymap-p entry state))
     ((null state)
      map)
     ((eq entry state)
      map)
...
(defun aiern-intercept-keymap-state (map)
...
A return value of t means all states."
  (let ((state (lookup-key map [intercept-state] map)))
    (cond
     ((keymapp state)
      (aiern-intercept-keymap-state state))
     ((eq state 'all)
      t)
     (t
      state))))

(defun aiern-overriding-keymap-state (map)
  "Return the state for the overriding keymap MAP.
A return value of t means all states."
  (let ((state (lookup-key map [override-state] map)))
    (cond
     ((keymapp state)
      (aiern-overriding-keymap-state state))
     ((eq state 'all)
      t)
     (t
      state))))

(defun aiern-send-leader ()
...
(defun aiern-set-leader (state key &optional localleader)
...
instead."
  (let* ((all-states '(normal insert visual replace operator motion emacs))
         (states (cond ((listp state) state)
                       ...
                       (t (list state))))
         (binding (if localleader 'aiern-send-localleader 'aiern-send-leader)))
    (dolist (state states)
      (aiern-global-set-key state key binding))))

(defmacro aiern-define-key (state keymap key def &rest bindings)
...
like those of `define-key'. For example:
    ...
    (aiern-define-key 'normal foo-map \"a\" 'bar)
...
global aiern keymap corresponding to the state(s) is used, meaning
the following lead to identical bindings:

    (aiern-define-key 'normal 'global \"a\" 'bar)
    (aiern-global-set-key 'normal \"a\" 'bar)

The symbol `local' may also be used, which corresponds to using
`aiern-local-set-key'. If a quoted symbol is used that is not
`global' or `local', it is assumed to be the name of a minor
mode, in which case `aiern-define-minor-mode-key' is used."
  (declare (indent defun))
  (cond ((member keymap '('global 'local))
         `(aiern-define-key* ,state ,keymap ,key ,def ,@bindings))
        ((and (consp keymap) (eq (car keymap) 'quote))
         `(aiern-define-minor-mode-key ,state ,keymap ,key ,def ,@bindings))
        (t
         `(aiern-delay ',(if (symbolp keymap)
                            `(and (boundp ',keymap) (keymapp ,keymap))
                          `(keymapp ,keymap))
              '(condition-case-unless-debug err
                   (aiern-define-key* ,state ,keymap ,key ,def ,@bindings)
                 (error
                  (message "error in aiern-define-key: %s"
                           (error-message-string err))))
            'after-load-functions t nil
            (format "aiern-define-key-in-%s"
                    ',(if (symbolp keymap) keymap 'keymap))))))
(defalias 'aiern-declare-key 'aiern-define-key)

(defun aiern-define-key* (state keymap key def &rest bindings)
  "Create a STATE binding from KEY to DEF for KEYMAP.
...
The use is nearly identical to `aiern-define-key' with the
exception that this is a function and not a macro (and so will
not be expanded when compiled which can have unintended
consequences). `aiern-define-key*' also does not defer any
bindings like `aiern-define-key' does using `aiern-delay'. This
...
its behavior more predictable."
  ...
  (let ((maps
             ...
             (mapcar
              (lambda (st)
                (cond ((eq keymap 'global)
                       (aiern-state-property st :keymap t))
                      ((eq keymap 'local)
                       (aiern-state-property st :local-keymap t))
                      (t
                       (aiern-get-auxiliary-keymap keymap st t t))))
    ...
    (dolist (map maps)
      (aiern-set-keymap-prompt map (keymap-prompt map)))))

(defun aiern-define-minor-mode-key (state mode key def &rest bindings)
  "Similar to `aiern-define-key' but the bindings are associated
...
aiern to use Emacs' built-in mechanisms to enable the bindings
automatically when MODE is active without relying on calling
`aiern-normalize-keymaps'. Another less significant difference is
that the bindings can be created immediately, because this
function only uses the symbol MODE and does not rely on its
value.

See `aiern-define-key' for the usage of STATE, KEY, DEF and
BINDINGS."
  (declare (indent defun))
  (let ((maps (mapcar
               (lambda (st)
                 (aiern-get-minor-mode-keymap st mode))
               (if (listp state) state (list state)))))
    (while key
      (dolist (map maps)
        (define-key map key def))
...
(defmacro aiern-add-hjkl-bindings (keymap &optional state &rest bindings)
  "Add \"h\", \"j\", \"k\", \"l\" bindings to KEYMAP in STATE.
Add additional BINDINGS if specified."
  (declare (indent defun))
  `(aiern-define-key ,state ,keymap
     "h" (lookup-key aiern-motion-state-map "h")
     "j" (lookup-key aiern-motion-state-map "j")
     "k" (lookup-key aiern-motion-state-map "k")
     "l" (lookup-key aiern-motion-state-map "l")
     ":" (lookup-key aiern-motion-state-map ":")
     ,@bindings))

;; may be useful for programmatic purposes
(defun aiern-global-set-key (state key def)
  "Bind KEY to DEF in STATE."
  (define-key (aiern-state-property state :keymap t) key def))

(defun aiern-local-set-key (state key def)
  "Bind KEY to DEF in STATE in the current buffer."
  (define-key (aiern-state-property state :local-keymap t) key def))

;; Advise these functions as they may activate an overriding keymap or
;; a keymap with state bindings; if so, refresh `aiern-mode-map-alist'.
(defadvice use-global-map (after aiern activate)
  "Refresh aiern keymaps."
  (aiern-normalize-keymaps))

(defadvice use-local-map (after aiern activate)
  "Refresh aiern keymaps."
  (aiern-normalize-keymaps))

(defmacro aiern-define-state (state doc &rest body)
  "Define an aiern state STATE.
...
- `:suppress-keymap' - if non-nil, effectively disables bindings to
  `self-insert-command' by making `aiern-suppress-map' the parent of
  the global state keymap.

The global keymap of this state will be `aiern-test-state-map',
the local keymap will be `aiern-test-state-local-map', and so on.
...
\(fn STATE DOC [[KEY VAL]...] BODY...)"
  ...
  (let* ((name (and (string-match "^\\(.+\\)\\(\\(?:.\\|\n\\)*\\)" doc)
         ...
         (doc (if (or (null doc) (string= doc "")) ""
                (format "\n%s" doc)))
         (toggle (intern (format "aiern-%s-state" state)))
    ...
    ;; macro expansion
    `(progn
       ;; Save the state's properties in `aiern-state-properties' for
...
cursor, or a list of the above." name))
       ...
       ,@(when suppress-keymap
           `((set-keymap-parent ,keymap aiern-suppress-map)))
...
\(That is, whether `aiern-state' is `%s'.)" name state)
         (and aiern-local-mode
              (eq (or state aiern-state) ',state)))
...
If ARG is nil, don't display a message in the echo area.%s" name doc)
         ...
         (cond
          ((and (numberp arg) (< arg 1))
           (setq aiern-previous-state aiern-state
                 aiern-state nil)
           (let ((aiern-state ',state))
             (run-hooks ',exit-hook)
             (setq aiern-state nil)
             (aiern-normalize-keymaps)
             ,@body))
          (t
           (unless aiern-local-mode
             (aiern-local-mode 1))
           (let ((aiern-next-state ',state)
                 input-method-activate-hook
                 input-method-deactivate-hook)
             (aiern-change-state nil)
             (setq aiern-state ',state)
             (aiern--add-to-alist 'aiern-previous-state-alist
                                ',state aiern-previous-state)
             (let ((aiern-state ',state))
               (aiern-normalize-keymaps)
               (if ',input-method
                   (activate-input-method aiern-input-method)
               ...
               (unless aiern-no-display
                 (aiern-refresh-cursor ',state)
                 (aiern-refresh-mode-line ',state)
                 (when (called-interactively-p 'any)
                   (redisplay)))
               ,@body
               (run-hooks ',entry-hook)
               (when (and aiern-echo-state
                          arg (not aiern-no-display) ,message)
                 (if (functionp ,message)
                     (funcall ,message)
                   (aiern-echo "%s" ,message))))))))

       (aiern-set-command-property ',toggle :keep-visual t)
       (aiern-set-command-property ',toggle :suppress-operator t)

       (aiern-define-keymap ,keymap nil
         :mode ,mode
         :func nil)

       (aiern-define-keymap ,local-keymap nil
         :mode ,local
         :local t
         :func nil)
